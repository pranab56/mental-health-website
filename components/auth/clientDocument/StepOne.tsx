...
import { DatePicker, Input, Label, SectionHeader, SelectDropdown } from "./FormPrimitives";
...
export default function StepOne({ data, setData, errors }: {
  ...
  const set = (key: keyof ClientIntakeFormData) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (key === "zip" && val.length > 5) return;
    setData((d) => ({ ...d, [key]: val }));
  };
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setData((d) => ({ ...d, profilePhoto: reader.result as string }));
      };
    ...
    <div key="profile">
      ...
      <div className="bg-gray-50 rounded-xl border border-gray-100 p-4 flex items-center gap-4 mb-5">
        <div className="relative w-14 h-14 rounded-full bg-gray-200 flex items-center justify-center shrink-0 overflow-hidden border border-gray-100 shadow-sm">
          {data.profilePhoto ? (
            <Image src={data.profilePhoto} alt="Profile" width={1000} height={1000} className="w-full h-full object-cover" />
          ) : (
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            {data.profilePhoto && (
              <button
                type="button"
                onClick={() => setData((d) => ({ ...d, profilePhoto: null }))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        ...
        <div><Label>Phone Number</Label><Input type='number' value={data.phone} onChange={set("phone")} placeholder="+1 (555) 000-0000" error={errors.phone} /></div>
        <div>
          <Label>Date of Birth</Label>
          <DatePicker
            value={data.dob}
            onChange={(dateString: string) => setData(d => ({ ...d, dob: dateString }))}
        ...
        <div>
          ...
          <SelectDropdown value={data.gender} onChange={(v) => setData((d) => ({ ...d, gender: v }))} placeholder="Select Gender" options={["Male", "Female", "Non-binary", "Other", "Prefer not to say"]} error={errors.gender} />
        </div>
        <div>
          <Label>Preferred Language</Label>
          <SelectDropdown value={data.language} onChange={(v) => setData((d) => ({ ...d, language: v }))} placeholder="English" options={["English", "Spanish", "French", "German", "Bengali", "Other"]} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <div><Label>Phone Number</Label><Input type='number' value={data.ecPhone} onChange={set("ecPhone")} placeholder="+1 (555) 000-0000" error={errors.ecPhone} /></div>
      </div>
      <div className="mb-5">
        <Label>Relationship</Label>
        <SelectDropdown value={data.ecRel} onChange={(v) => setData((d) => ({ ...d, ecRel: v }))} placeholder="Select Relationship" options={["Spouse", "Parent", "Sibling", "Friend", "Other"]} error={errors.ecRel} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div><Label>Zip Code</Label><Input type='number' value={data.zip} onChange={set("zip")} placeholder="62704" error={errors.zip} /></div>
